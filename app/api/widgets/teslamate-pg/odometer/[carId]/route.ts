...
import { getOdometer } from "@/lib/widgets/teslamate-odometer/query";
...
export async function GET(
  ...
  try {
    return NextResponse.json(await getOdometer(id));
  } catch (e) {
    if (e instanceof TeslamatePostgresNotConfigured) {
      return NextResponse.json({ error: e.message }, { status: 503 });
    }
    const msg = e instanceof Error ? e.message : String(e);
    console.error("[dash:teslamate-odometer] route failed", msg);
